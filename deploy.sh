...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=shermanng10/multi-client:$SHA
kubectl set image deployments/server-deployment server=shermanng10/multi-server:$SHA
kubectl set image deployments/worker-deployment worker=shermanng10/multi-worker:$SHA
